...
export async function registerRoutes(
  ...
  const createExpenseSchema = z.object({
    expenseType: z.enum(["salary", "hammali", "grading_charges", "general_expenses", "cost_of_goods_sold", "tds", "interest_on_loan", "electricity_charges", "chemical_spray_charges", "farmer_advance", "farmer_freight", "merchant_advance", "farmer_loan", "other_advance", "asset_purchase", "bank_charges", "bank_penalty_charges", "insurance", "loan_principal"]),
  ...
  app.post("/api/expenses", requireAuth, requireEditAccess, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const advanceTypes = ['farmer_advance', 'farmer_freight', 'merchant_advance', 'farmer_loan', 'other_advance'];
  ...
  app.get("/api/farmer-ledger/:id/transactions", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const { id } = req.params;
      const fy = parseInt(req.query.fy as string, 10);
      if (isNaN(fy)) {
      ...
      const coldStorageId = getColdStorageId(req);
      const result = await storage.getFarmerTransactions(id, coldStorageId, fy);
      res.json(result);
    } catch (error) {
    ...
    try {
      ...
      for (const sale of sales) {
        ...
        const row = [
          ...
          sale.hammali || "",
          sale.pricePerBag,
          (sale.coldStorageCharge || 0) + (sale.extraDueToMerchant || 0),
