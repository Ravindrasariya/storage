...
- **Chamber Fill (exit-history derived)**: `chambers.currentFill` is no longer maintained as a running counter. It is recomputed from exit history (`lot.size − Σ non-reversed exit_history.bagsExited`) inside `getDashboardStats` and persisted back to the column. Floor-level fill in `getFloorCapacityByChamber` uses the same exit-history-based `physicalRemaining`. No code should increment/decrement `currentFill` on lot creation, exits, exit reversals, sale reversals, or lot-size edits — those counter mutations were removed. The column is always refreshed from truth when the dashboard is loaded.
