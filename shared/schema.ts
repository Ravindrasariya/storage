...
export const coldStorages = pgTable("cold_storages", {
  ...
  nextColdStorageBillNumber: integer("next_cold_storage_bill_number").notNull().default(1), // DEPRECATED — no longer read or written by app code. Bill # auto-assignment now uses MAX(coldStorageBillNumber) + 1 scoped to (cold_storage, year(soldAt)), so reversing the most recent bill # frees that trailing slot for the next blank-input sale; interior gaps from older reversals are intentionally NOT refilled. Column kept to avoid a destructive migration; safe to drop in a follow-up once stable.
