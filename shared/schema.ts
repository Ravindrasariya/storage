...
export const coldStorages = pgTable("cold_storages", {
  ...
  nextExitBillNumber: integer("next_exit_bill_number").notNull().default(1), // DEPRECATED (Task #354) — was a LIFETIME counter that never reset, so it cannot express a per-entry-year series. No longer read or written by app code. Exit/Nikasi bill # auto-assignment now uses MAX(exit_history.bill_number) + 1 over non-reversed rows scoped to (cold_storage, STOCK ENTRY year), matching the CS-bill rule below. Column kept to avoid a destructive migration; safe to drop in a follow-up once stable.
  nextColdStorageBillNumber: integer("next_cold_storage_bill_number").notNull().default(1), // DEPRECATED — no longer read or written by app code. Bill # auto-assignment now uses MAX(coldStorageBillNumber) + 1 scoped to (cold_storage, STOCK ENTRY year — from sales_history.entry_date, falling back to lots.created_at; NOT year(soldAt), see Task #354), so reversing the most recent bill # frees that trailing slot for the next blank-input sale; interior gaps from older reversals are intentionally NOT refilled. Column kept to avoid a destructive migration; safe to drop in a follow-up once stable.
...
export const farmerAdvanceFreight = pgTable("farmer_advance_freight", {
  id: varchar("id").primaryKey(),
  coldStorageId: varchar("cold_storage_id").notNull(),
